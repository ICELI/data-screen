...
function Rings(para, doc){
	...
	function render(){
		...
		if(this.data && (this.data.length != this.vNodes.length)){
			...
			this.data && config.colors && (config.colors.length >= this.data.length) && this.data.forEach(function(col, idx){
				...
				if(idx % 2 == 0){
					html += '<li style="position:absolute; left:0; width:auto; height:auto; margin-top:' + (idx / 2) * 70 + 'px">' + label + '</li>';
				}else{
					html += '<li style="position:absolute; right:0; width:auto; height:auto; margin-top:' + ((idx - 1) / 2) * 70 + 'px">' + label + '</li>';
				}
			});
			html += '<div style="display:inline-block; width:' + config.size + 'px; height:' + config.size + 'px;">' + str + '</div></div>';
	...
	function VNode(para){
		...
		this.render = function(config){
			...
			var col = this.value;
			var odd = (config.dir == 'DIR') ? (this.ide % 2 == 0) : (config.dir == 'LEFT') ? false : true;
